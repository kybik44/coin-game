...
function showFireworks() {
  ...
  for (let i = 0; i < GAME_CONFIG.FIREWORKS_PARTICLES; i++) {
    ...
    firework.style.setProperty(
      "--x",
      `${
        Math.random() * GAME_CONFIG.FIREWORKS_SPREAD -
        GAME_CONFIG.FIREWORKСS_SPREAD / 2
      }px`
    );
    firework.style.setProperty(
      "--y",
      `${
        Math.random() * GAME_CONFIG.FIREWORKСS_SPREAD -
        GAME_CONFIG.FIREWORKСS_SPREAD / 2
      }px`
    );
    firework.style.backgroundColor = `hsl(${
      Math.random() * GAME_CONFIG.FIREWORKСS_HUE_RANGE +
      GAME_CONFIG.FIREWORKСS_HUE_MIN
    }, 100%, 50%)`;
...
async function collectCoins(userId) {
  try {
    ...
    window.Telegram?.WebApp?.showAlert(
      error.message || "Error collecting coins"
    );
  }
}
...
async function claimReward(userId) {
  try {
    const response = await fetch(
      `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.CLAIM}`,
      {
        method: "POST",
...
function generateCardValues() {
  ...
  if (Math.random() < GAME_CONFIG.WINNING_CHANCE) {
    const valuableCardValue =
      GAME_CONFIG.POSSIBLE_REWARDS[
